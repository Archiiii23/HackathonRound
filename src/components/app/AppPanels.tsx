...
import { activityQuery, meQuery, qk, workspaceSummaryQuery } from "@/lib/queries";
...
export function NotificationsSheet({
  ...
  React.useEffect(() => {
    ...
    try {
      const stored = window.localStorage.getItem("devcollab:notif-read");
  ...
  function persist(next: Set<string>) {
    setReadIds(next);
    try {
      window.localStorage.setItem("devcollab:notif-read", JSON.stringify(Array.from(next)));
    } catch {
      /* noop */
    }
  }
  ...
  function markAll() {
    ...
    (events.data?.events ?? []).forEach((e) => next.add(e.id));
    persist(next);
  ...
  const unreadCount = all.filter((e) => !readIds.has(e.id)).length;
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="flex w-full flex-col gap-0 p-0 sm:max-w-md">
            ...
            <ul className="divide-y divide-border/60">
              {all.map((a) => {
                ...
                return (
                  ...
                  >
                    <Avatar initials={a.actor.initials} color={a.actor.avatarColor} size={30} />
        ...
        <footer className="border-t border-border bg-muted/20 px-5 py-2.5 text-[11px] text-muted-foreground">
          Showing the latest {all.length} events
        </footer>
      </SheetContent>
    </Sheet>
  );
...
export function InviteDialog({
  ...
  const inviteUrl = React.useMemo(() => {
    if (typeof window === "undefined") return "";
    const code = btoa((user?.id ?? "demo").slice(0, 12) + ":" + (Date.now() % 1_000_000));
          ...
          <form onSubmit={sendInvites} className="space-y-3">
            ...
            <div>
              ...
              <div className="mt-1.5 grid grid-cols-2 gap-2">
                {(
                  [
                    { id: "member", title: "Member", sub: "Can view & edit content" },
                    { id: "admin", title: "Admin", sub: "Plus billing & settings" },
                  ] as const
                ).map((r) => (
                  <button
                    type="button"
            ...
            <div className="flex items-center justify-between gap-2 pt-1">
              <span className="text-[11px] text-muted-foreground">
                We&apos;ll send invitations as soon as you confirm.
              </span>

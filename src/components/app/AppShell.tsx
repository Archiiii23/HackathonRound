...
import { Link, useNavigate, useRouter, useRouterState, useMatches } from "@tanstack/react-router";
import { useQuery, useQueryClient } from "@tanstack/react-query";
import { Logo } from "@/components/brand/Logo";
import { Button } from "@/components/ui/button";
import { api, type AiPlatform, type AiResult } from "@/lib/api";
...
const QUICK_PROMPTS: {
  id: string;
  kind: "architecture" | "standup" | "refactor" | "db";
  title: string;
  sub: string;
  emoji: string;
}[] = [
  {
    id: "architecture",
    kind: "architecture",
    title: "Explain workspace architecture",
  ...
  },
  {
    ...
    sub: "From recent activity & PRs.",
    emoji: "📋",
  },
  {
    id: "refactor",
    kind: "refactor",
    title: "Optimize board reducer",
    sub: "Cleaner status updates.",
    emoji: "⚡",
  },
  {
    id: "db",
    kind: "db",
    title: "Setup D1 migrations",
    sub: "Drizzle workflow tips.",
    emoji: "🗃️",
  },
];
...
function AiHub({ onClose, projectId }: { onClose: () => void; projectId?: string }) {
  ...
  async function runPrompt(
    kind: "architecture" | "standup" | "refactor" | "db" | "chat",
    userPrompt?: string,
  ) {
    setLoading(true);
    setResponse(null);
    try {
      ...
      const result = await api.ai({ kind, platform, prompt: promptText, projectId });
      setResponse(result);
      if (result.performedAction) {
        toast.success(`AI: ${result.performedAction}`);

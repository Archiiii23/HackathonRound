    ...
    <div className="mx-auto max-w-[1280px] px-6 py-6">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="sm">Group: Status</Button>
        </div>
        <div className="hidden gap-1 rounded-md border border-border p-0.5 md:flex">
          {["Board", "List", "Calendar"].map((v, i) => (
            <button key={v} className={`h-7 rounded px-2.5 text-xs font-medium transition-colors ${i === 0 ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>{v}</button>
          ))}
        </div>
          ...
          return (
            <div key={col.key} className="flex flex-col rounded-lg bg-muted/40">
              <div className="sticky top-14 z-10 flex items-center justify-between border-b border-border bg-muted/80 px-3 py-2.5 backdrop-blur">
                ...
                <Button variant="ghost" size="icon" className="h-7 w-7" aria-label="Add card"><Plus className="h-3.5 w-3.5" /></Button>
              </div>
              <div
              ...
              >
                  ...
                  >
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        {t.due && <span className="inline-flex items-center gap-1"><CalIcon className="h-3 w-3" />{t.due}</span>}
                        {t.comments > 0 && <span className="inline-flex items-center gap-1"><MessageSquare className="h-3 w-3" />{t.comments}</span>}
                      </div>
                ...
                {items.length === 0 && (
                  <div className="rounded-md border border-dashed border-border p-4 text-center text-xs text-muted-foreground">Drop tasks here</div>
                )}
              </div>
            </div>
          );

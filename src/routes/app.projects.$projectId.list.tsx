      ...
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="sm" className="gap-1.5">
            Sort: Updated <ChevronDown className="h-3.5 w-3.5" />
          </Button>
        </div>
              ...
              <tr className="text-left text-xs uppercase tracking-wider text-muted-foreground">
                <th className="w-10 px-3 py-2.5">
                  <input
                    ...
                    onChange={(e) =>
                      setSelected(
                        e.target.checked ? new Set(mockTasks.map((t) => t.id)) : new Set(),
                      )
                    }
            ...
            <tbody>
              {mockTasks.map((t) => {
                ...
                return (
                  <tr
                    key={t.id}
                    className={`group border-t border-border transition-colors ${isSel ? "bg-primary/5" : "hover:bg-muted/40"}`}
                  >
                    ...
                    <td className="hidden py-3 md:table-cell">
                      <InlineSelect
                        value={statusMeta[t.status].label}
                        color={statusMeta[t.status].color}
                      />
                    </td>
                    <td className="hidden py-3 lg:table-cell">
                      <InlineSelect
                        value={priorityMeta[t.priority].label}
                    ...
                    <td className="py-3 pr-3 text-right opacity-0 transition-opacity group-hover:opacity-100">
                      <Button size="icon" variant="ghost" className="h-7 w-7" aria-label="More">

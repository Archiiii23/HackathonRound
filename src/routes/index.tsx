        ...
        <div className="grid items-center gap-12 md:grid-cols-12">
          <div className="md:col-span-7 animate-[var(--animate-rise-in)]">
            ...
            <p className="mt-5 max-w-xl text-lg text-muted-foreground">
              DevCollab unifies your tasks, wiki, snippets, and code review — with AI that drafts
...
function HeroMock() {
  return (
    <div className="relative animate-[var(--animate-scale-in)]">
      <div
        className="absolute -inset-4 rounded-2xl bg-gradient-to-br from-primary/30 to-transparent blur-2xl"
        aria-hidden
      />
                  ...
                  <div key={j} className="rounded-md border border-border bg-card p-2 shadow-sm">
                    ...
                    <div className="mt-2 flex items-center gap-1">
                      ...
                      <div
                        ...
                        style={{ background: "oklch(0.7 0.15 155)" }}
                      />
                    </div>
                  </div>
                ))}
    ...
    <section className="border-b border-border bg-muted/30">
      <div className="mx-auto flex max-w-[1280px] flex-wrap items-center justify-between gap-6 px-6 py-8">
        ...
        <div className="flex flex-wrap items-center gap-x-10 gap-y-3">
          {logos.map((l) => (
            <span key={l} className="font-display text-sm font-semibold text-muted-foreground/80">
              {l}
            </span>
          ))}
        </div>
            ...
            <ul className="mt-6 space-y-3 text-sm">
              {[
                ...
                <li key={i} className="flex items-start gap-2">
                  <Zap className="mt-0.5 h-4 w-4 text-primary" /> {x}
                </li>
              ))}
            </ul>
          </div>
          <div className="surface-card overflow-hidden p-0">
            <div className="border-b border-border bg-muted/40 px-4 py-2.5 text-xs font-mono text-muted-foreground">
              ai/standup.md
            </div>
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {quotes.map((q, i) => (
            <figure key={i} className="relative surface-card p-6">
              <div
                aria-hidden
                className="pointer-events-none absolute inset-0 rounded-[inherit] p-px bg-gradient-to-br from-primary/30 via-transparent to-transparent [mask:linear-gradient(white,white)_content-box,linear-gradient(white,white)] [mask-composite:exclude]"
              />
              <blockquote className="text-sm leading-relaxed">"{q.quote}"</blockquote>
              <figcaption className="mt-5 flex items-center gap-3 text-sm">
                <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-accent text-xs font-semibold">
                  {q.name
                    .split(" ")
                    .map((n) => n[0])
                    .join("")}
